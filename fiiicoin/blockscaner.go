...
func (bs *FIIIBlockScanner) scanBlock(height uint64) (*Block, error) {
...
func (bs *FIIIBlockScanner) newBlockNotify(block *Block, isFork bool) {
...
//fiiicoin 1M的区块链可以容纳3000笔交易，批量多线程处理，速度更快
...
func (bs *FIIIBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *FIIIBlockScanner) extractTransaction(trx *Transaction, result *ExtractResult, scanAddressFunc openwallet.BlockScanAddressFunc) {
...
func (bs *FIIIBlockScanner) extractTxInput(trx *Transaction, result *ExtractResult, scanAddressFunc openwallet.BlockScanAddressFunc) ([]string, decimal.Decimal) {
	...
	for i, output := range trx.Vins {
		...
		amount := common.IntToDecimals(int64(output.Amount), bs.wm.Decimal()).String()
		addr := output.Addr
...
//ExtractTxInput 提取交易单输入部分
func (bs *FIIIBlockScanner) extractTxOutput(trx *Transaction, result *ExtractResult, scanAddressFunc openwallet.BlockScanAddressFunc) ([]string, decimal.Decimal) {
...
func (bs *FIIIBlockScanner) newExtractDataNotify(height uint64, extractData map[string]*openwallet.TxExtractData) error {
...
//func (bs *FIIIBlockScanner) SaveRechargeToWalletDB(height uint64, list []*openwallet.Recharge) error {
//
//	for _, r := range list {
//
...
//func (bs *FIIIBlockScanner) DropRechargeRecords(accountID string) error {
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
//func (bs *FIIIBlockScanner) DeleteRechargesByHeight(height uint64) error {
//
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
func (bs *FIIIBlockScanner) SaveUnscanRecord(record *UnscanRecord) error {
...
//func (bs *FIIIBlockScanner) GetWalletByAddress(address string) (*openwallet.Wallet, bool) {
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
func (wm *WalletManager) GetTxIDsInMemPool() ([]string, error) {
	...
	result, err := wm.WalletClient.Call("GetAllTxInMemPool", nil)
	if err != nil {
		return nil, err
	}

	if !result.IsArray() {
		return nil, fmt.Errorf("no query record")
	}
